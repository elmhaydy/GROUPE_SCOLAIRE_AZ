(() => {
  ...
  const cfg = window.AZ_ELEVE || {};
  const URL_NIVEAUX = cfg.urlNiveaux || "";
  const URL_GROUPES = cfg.urlGroupes || "";

  if (!URL_NIVEAUX || !URL_GROUPES) {
    // Si tu as oublié d'injecter window.AZ_ELEVE dans le template
    console.warn("[AZ] URLs AJAX manquantes (AZ_ELEVE).");
    return;
  }
  ...
  async function fetchJSON(url) {
    ...
    if (!res.ok) {
      console.warn("[AZ] AJAX error:", res.status, url);
      return {};
    }
