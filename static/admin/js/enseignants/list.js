(() => {
  const formEl   = document.getElementById("ensFilterForm");
  ...
  if (!URL_NIVEAUX || !URL_GROUPES || !URL_PERIODES) {
    console.warn("[AZ] URLs AJAX manquantes (AZ_ENS).");
    return;
  }

  // ✅ Si tu veux recharge automatique après changement
  const AUTO_SUBMIT = false;

  function submitIfNeeded() {
    if (!AUTO_SUBMIT || !formEl) return;
  ...
  function setOptions(selectEl, items, placeholderText) {
    ...
    (items || []).forEach(it => {
      ...
      opt.textContent = it.label || it.nom || "";
  ...
  async function loadNiveaux() {
    ...
    setOptions(niveauEl, data.results || [], "Tous les niveaux");
  }
  ...
  async function loadGroupes() {
    ...
    const data = await fetchJSON(url);
    setOptions(groupeEl, data.results || [], "Tous les groupes");
  }
  ...
  // optionnel : si tu veux auto submit sur groupe/periode aussi
  groupeEl.addEventListener("change", submitIfNeeded);
  periodeEl.addEventListener("change", submitIfNeeded);
